...
echo "#export EXTERNAL_NETWORK=external" >> ~/admin-openrc.sh
echo "#export PROJECT_ID=\$(openstack project list | grep \$OS_PROJECT_NAME | awk '{print \$2}')" >> ~/admin-openrc.sh
echo "#export ENDPOINT_ID=\$(openstack endpoint list | grep \ orchestration | grep internal | awk '{print \$2}')" >> ~/admin-openrc.sh
